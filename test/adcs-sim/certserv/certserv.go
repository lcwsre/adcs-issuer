...
func (c *Certserv) HandleCertnewCer(w http.ResponseWriter, req *http.Request) {
	...
	file, err := os.ReadFile(certFileName)
	if err == nil {
	...
	file, err = os.ReadFile(csrFileName)
	if err != nil {
	...
	if orders.unauthorized {
		...
		file, _ := os.ReadFile(tmplUnauthorized)
...
func (c *Certserv) HandleCertfnshAsp(w http.ResponseWriter, req *http.Request) {
	...
	if orders.delay > 0 || orders.reject {
		...
		err = os.WriteFile(csrFileName, []byte(bodyCsr[0]), 0644)
		if err != nil {
...
func (c *Certserv) initRootCert() error {
	...
	if block == nil {
		return fmt.Errorf("Cannot decode root CA cert PEM")
	}
	...
	if keyBlock == nil {
		return fmt.Errorf("Cannot decode root CA key PEM")
	}
	c.caKey, err = x509.ParsePKCS1PrivateKey(keyBlock.Bytes)
	if err != nil {
