...
import (
	...
	"github.com/lcwsre/adcs-issuer/test/adcs-sim/certserv"
)
...
func generateServerCertificate(cs *certserv.Certserv, ips *string, dns *string) error {
	...
	err = os.WriteFile(serverCsr, pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE REQUEST", Bytes: csrBytes}), 0644)
	if err != nil {
	...
	err = os.WriteFile(serverPem, []byte(certPem), 0644)
	if err != nil {
