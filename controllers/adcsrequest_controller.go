...
import (
	...
	"github.com/lcwsre/adcs-issuer/issuers"
)
...
// +kubebuilder:rbac:groups=adcs.certmanager.lcwsre.io,resources=adcsrequests,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=adcs.certmanager.lcwsre.io,resources=adcsrequests/status,verbs=get;update;patch
...
func (r *AdcsRequestReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		log.Error(err, "Failed to get CertificateRequest")
		return ctrl.Result{}, err
	}
	...
	case api.Ready:
		...
		cr.Status.CA = caCert
		if err := r.CertificateRequestController.SetStatus(ctx, &cr, cmmeta.ConditionTrue, cmapi.CertificateRequestReasonIssued, "ADCS request successfull"); err != nil {
			log.Error(err, "Failed to update CertificateRequest status, will retry")
			return ctrl.Result{Requeue: true, RequeueAfter: issuer.RetryInterval}, nil
		}
	case api.Rejected:
